...
import { MessageSquare, User, Headphones, Loader2, Mic } from "lucide-react";
import { useApp } from "../../context/AppContext";

function SentimentDot({ score }: { score: number }) {
  const color = score > 0.1 ? "bg-[#2d6a4f]" : score < -0.1 ? "bg-red-400" : "bg-slate-500";
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700/50">
        <div className="flex items-center gap-2">
          ...
          {isProcessing && (
            <div className="flex items-center gap-1.5 text-xs text-[#95d5b2]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin">
        {transcripts.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center py-12">
            <div className="w-16 h-16 rounded-full bg-[#2d6a4f]/10 border border-[#2d6a4f]/30 flex items-center justify-center mb-4">
              <Mic className="w-7 h-7 text-[#95d5b2]" />
            </div>
            <p className="text-white text-sm font-medium mb-1">Listening for Speech</p>
            ...
            return (
              ...
              >
                <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${
                  isAgent ? "bg-[#2d6a4f]" : "bg-slate-700"
                }`}>
                  {isAgent
                ...
                <div className={`flex flex-col gap-1 max-w-[75%] ${isAgent ? "items-end" : "items-start"}`}>
                  <div className={`flex items-center gap-1.5 ${isAgent ? "flex-row-reverse" : "flex-row"}`}>
                    <span className={`text-xs font-medium ${isAgent ? "text-[#95d5b2]" : "text-slate-400"}`}>
                  ...
                  <div className={`rounded-xl px-3.5 py-2.5 text-sm leading-relaxed ${
                    isAgent
                      ? "bg-[#2d6a4f]/20 border border-[#2d6a4f]/30 text-slate-200 rounded-tr-sm"
